...
namespace MailChimp.Api.Net.Services
{
  internal static class Authenticate
  {
      ...
      if (id != "")
      {
        if (subCategory != "")
        {
          if (param2 != "")
          {
            if (subChildCategory != "")
            {
              return String.Format("https://{0}.api.mailchimp.com/3.0/{1}/{2}/{3}/{4}/{5}", dataCenter, targetType, id,
                                   subCategory, param2, childSubType);
            }
            return String.Format("https://{0}.api.mailchimp.com/3.0/{1}/{2}/{3}/{4}", dataCenter, targetType, id,
                                 subCategory, param2);
          }
          return String.Format("https://{0}.api.mailchimp.com/3.0/{1}/{2}/{3}", dataCenter, targetType, id,
                               subCategory);
        }
    ...
    /// <summary>
    /// Return the endpoint to caller method
    /// <param name="client" optional>Expects id for particular list/campaign etc</param>
    /// </summary>
